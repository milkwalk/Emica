...
public class MetaTextTask implements Runnable {
    ...
    public void run() {
        ...
        if (track != null) {
            ...
            String timeLeft = OnEnable
                    .convertSecondsToString((int) ((track.getDuration() - track.getPosition()) / 1000L));
            String totalTime = OnEnable.convertSecondsToString((int) (track.getDuration() / 1000L));
            Emica.getBot().changePresence(StatusType.ONLINE, ActivityType.LISTENING, text.replace("%song_name%", title)
                    .replace("%time_left%", timeLeft).replace("%total_time%", totalTime));
        }

        if (Emica.getBot().getConnectedVoiceChannels().size() == 0) {
            Emica.getBot().getGuildByID(Emica.getServerID()).getVoiceChannelByID(Emica.getVoiceChannelID()).join();
        }
    }
}
